...
from degradations import GaussianBlurNoise,GaussianNoise,GaussianBlur,RandomMask,RandomMask2,RandomMask2Noise,MRISamplerGaussianNoise1,NoDegradation,MRISamplerGaussianNoise2
from datasets_prep.patalogy_datasets import Data_from_numpy,numpy_microscopy_new
...
def sample_and_test(args):
    ...
    dataset = microscopy_new_translate(dataroot=args.data_dir,phase='test',input_selection=args.input_selection,input_channels=args.input_channels,out_channels=args.out_channels,sum_input=args.sum_input)
    ...
    ckpt = torch.load('{}/{}/netG_{}.pth'.format(args.checkpoint_dir, args.exp, args.epoch_id), map_location=device)
    ...
    im_res = np.zeros([len(dataset),2,args.image_size, args.image_size])
    
    # save_dir = "./generated_samples/{}".format(args.dataset)
    save_dir = "{}/results/{}_epoch_{}".format(args.checkpoint_dir, args.exp, args.epoch_id)
    
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    
    total_im=0
    for iteration, (x_test , y_test) in enumerate(data_loader):
        real_data = y_test.to(device, non_blocking=True)
        source_data = x_test.to(device, non_blocking=True)
        ...
        save_im = torch.cat((real_data, fake_sample),axis=-1)
        ...
        # fake_sample_ddim=fake_sample_ddim.cpu().numpy().squeeze()
        real_data=real_data.cpu().numpy().squeeze()

        batch_size_cur= real_data.shape[0]
        

        im_res[total_im:total_im+batch_size_cur,0] = real_data
        im_res[total_im:total_im+batch_size_cur,1] = fake_sample
        # im_res[total_im:total_im+batch_size_cur,2] = fake_sample_ddim
